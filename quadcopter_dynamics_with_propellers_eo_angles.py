...
def input_update(w,rv,dv,e_pre,e_integral_pre,rv1,dv1,e_pre1,e_integral_pre1):
    ...
    w[0][0] = np.clip(100+(cof1)*e+cof2*e_integral+cof3*e_derivative+(cof11)*e1+cof21*e_integral1+cof31*e_derivative1,0,500)
    w[1][0] = np.clip(100+(cof1)*e+(cof2)*e_integral+(cof3)*e_derivative+(cof11)*e1+cof21*e_integral1+cof31*e_derivative1,0,500)
    w[2][0] = np.clip(0+(cof1)*e+(cof2)*e_integral+(cof3)*e_derivative+(cof11)*e1+cof21*e_integral1+cof31*e_derivative1,0,500)
...
if __name__=="__main__":
    ...
    ppitch = 0.0
    pyaw = 0.0
    proll = proll + Ts * va0[0][0]
    ppitch = ppitch + Ts * va0[1][0]
    pyaw = pyaw + Ts * va0[2][0]
